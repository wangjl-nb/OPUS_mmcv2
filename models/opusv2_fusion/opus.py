...
class OPUSV2Fusion(MVXTwoStageDetector):
    def __init__(self,
                 use_grid_mask=True,
                 data_aug=None,
                 stop_prev_grad=0,
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        if self.training and self.stop_prev_grad > 0:
            ...
            for lvl in range(len(all_img_feats[0])):
                ...
                img_feat = torch.cat(
                    [feat[lvl].reshape(B, -1, num_views, C, H, W) for feat in all_img_feats],
                    dim=1)
    ...
    def _forward(self, inputs, data_samples=None):
        ...
        img_feats = None if not self.with_img_backbone else self.extract_img_feat(img, img_metas)
        pts_feats = None if not self.with_pts_backbone else self.extract_pts_feat(points)
        if pts_feats is not None:
            pts_feats = self.final_conv(pts_feats[0])
        return self.pts_bbox_head(mlvl_feats=img_feats, pts_feats=pts_feats, points=points, img_metas=img_metas)
