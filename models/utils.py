...
def debug_is_finite(name, tensor_like):
    flag = os.getenv('OPUS_DEBUG_FINITE', '')
    if flag.lower() not in ('1', 'true', 'yes', 'y'):
        return
    ...
    def _check(name, value):
        ...
        if isinstance(value, (list, tuple)):
            for idx, item in enumerate(value):
                _check(f'{name}[{idx}]', item)
            return
        if isinstance(value, np.ndarray):
            if value.size == 0 or value.dtype.kind not in ('f', 'c'):
                return
            ...
            if nan_count or inf_count:
                ...
                stats = ''
                if finite.size > 0:
                    stats = (f' min={finite.min():.6g} max={finite.max():.6g} '
                             f'mean={finite.mean():.6g}')
                print(f'[OPUS_DEBUG_FINITE] {name} has nan={nan_count} inf={inf_count}{stats}')
        ...
        if isinstance(value, torch.Tensor):
            ...
            if nan_count or inf_count:
                ...
                if finite.numel() > 0:
                    ...
                    stats = (f' min={finite_f.min().item():.6g} '
                             f'max={finite_f.max().item():.6g} '
                             f'mean={finite_f.mean().item():.6g}')
                print(f'[OPUS_DEBUG_FINITE] {name} has nan={nan_count} inf={inf_count}{stats}')
                raise FloatingPointError(f'Non-finite values found in {name}')
            return

    _check(name, tensor_like)
