...
def msmv_sampling(mlvl_feats, sampling_locations, scale_weights):
    if len(mlvl_feats) == 4 and MSMV_CUDA:
        ...
        if out_dtype != torch.float32 or sampling_locations.dtype != torch.float32 or scale_weights.dtype != torch.float32:
            ...
            out = MSMVSamplingC2345.apply(*mlvl_feats, sampling_locations, scale_weights)
        ...
        if out_dtype != torch.float32 or sampling_locations.dtype != torch.float32 or scale_weights.dtype != torch.float32:
            ...
            sampling_locations = sampling_locations.float()
            scale_weights = scale_weights.float()
            out = MSMVSamplingC23456.apply(*mlvl_feats, sampling_locations, scale_weights)
            return out.to(out_dtype)
        return MSMVSamplingC23456.apply(*mlvl_feats, sampling_locations, scale_weights)
    else:
        target_dtype = mlvl_feats[0].dtype
        if sampling_locations.dtype != target_dtype:
            sampling_locations = sampling_locations.to(target_dtype)
        if scale_weights.dtype != target_dtype:
            scale_weights = scale_weights.to(target_dtype)
