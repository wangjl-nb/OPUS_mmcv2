...
def _build_class_names(num_classes=None, class_names=None, empty_label=None):
    if class_names is not None:
        ...
        if num_classes is None:
            num_classes = empty_label + 1 if empty_label is not None else len(occ_class_names)
        names = [f'class_{i}' for i in range(num_classes)]

    if empty_label is None:
        empty_label = num_classes - 1
    if 0 <= empty_label < len(names):
        names[empty_label] = 'free'
    return names, num_classes, empty_label


def _ensure_occ_size(sem, occ_size):
    sem = np.asarray(sem)
    if sem.ndim == 3:
        return sem
    if occ_size is None:
        raise ValueError('occ_size must be provided when sem_pred/sem_gt is not 3D.')
    return np.reshape(sem, occ_size)


def process_one_sample_custom(sem_pred, lidar_rays, output_origin, pc_range, voxel_size, free_id):
    T = output_origin.shape[1]
    pred_pcds_t = []
    ...
    lidar_tindex = torch.zeros([1, lidar_rays.shape[0]])
    occ_shape = [1] + list(occ_pred.shape[-3:])
...
def calc_metrics_custom(pcd_pred_list, pcd_gt_list, class_names, free_id):
    thresholds = [1, 2, 4]
    ...
    for pcd_pred, pcd_gt in zip(pcd_pred_list, pcd_gt_list):
        for j, threshold in enumerate(thresholds):
            ...
            for cls_id in range(len(class_names)):
                ...
                pred_cnt_i = cls_mask_pred.sum()
                if j == 0:
                    gt_cnt[cls_id] += gt_cnt_i
                    pred_cnt[cls_id] += pred_cnt_i

                tp_cls = cls_mask_gt & cls_mask_pred
                tp_mask = np.logical_and(tp_cls, tp_dist_mask)
                tp_cnt[j][cls_id] += tp_mask.sum()

    iou_list = []
    for j in range(len(thresholds)):
        iou = tp_cnt[j] / (gt_cnt + pred_cnt - tp_cnt[j])
        if 0 <= free_id < len(class_names):
            iou = np.delete(iou, free_id)
        iou_list.append(iou)

    return iou_list


def main_custom(sem_pred_list,
                sem_gt_list,
                lidar_origin_list,
                occ_size=None,
                pc_range=None,
    ...
    class_names, num_classes, free_id = _build_class_names(
    ...
    for sem_pred, sem_gt, lidar_origins in tqdm(
        ...
        if occ_size is None:
            occ_size = sem_pred.shape if np.asarray(sem_pred).ndim == 3 else sem_gt.shape

        sem_pred = _ensure_occ_size(sem_pred, occ_size)
        sem_gt = _ensure_occ_size(sem_gt, occ_size)

        pcd_pred = process_one_sample_custom(
            sem_pred, lidar_rays, lidar_origins, pc_range, voxel_size, free_id)
        pcd_gt = process_one_sample_custom(
            sem_gt, lidar_rays, lidar_origins, pc_range, voxel_size, free_id)

        valid_mask = (pcd_gt[:, 0].astype(np.int32) != free_id)
        pcd_pred = pcd_pred[valid_mask]
        pcd_gt = pcd_gt[valid_mask]
    ...
    iou_list = calc_metrics_custom(pcd_pred_list, pcd_gt_list, class_names, free_id)
    rayiou = np.nanmean(iou_list)
    rayiou_0 = np.nanmean(iou_list[0])
    rayiou_1 = np.nanmean(iou_list[1])
    rayiou_2 = np.nanmean(iou_list[2])
    ...
    if 0 <= free_id < len(report_names):
        report_names.pop(free_id)

    for i, name in enumerate(report_names):
        table.add_row([
            name,
            iou_list[0][i], iou_list[1][i], iou_list[2][i]
        ], divider=(i == len(report_names) - 1))
