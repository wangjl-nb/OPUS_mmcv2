...
feature_dims = 256  # PCA-compressed semantic feature dimension.
...
pts_middle_encoder = dict(
    ...
    return_middle_feats=True,  # Required by TPV branch.
)
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        transformer=dict(
            ...
            feature_dims=feature_dims,  # Parallel semantic feature branch dims.
            score_mode='binary_occ',  # Use score head only for occupancy gating.
            occ_out_channels=occ_out_channels,  # Emit a single occupiedness logit per refined point.
            ...
            use_pts_sampling=False,  # Disable point sampling path.
            use_tpv_sampling=True,  # Enable TPV sampling path.
            ...
            query_allocator=dict(
                ...
                context_ratio=0.5,  # Reserve more detail queries for tail/small-object regions.
        ...
        feature_supervision=dict(
            enabled=True,  # Learn PCA-compressed semantic features in parallel.
            feature_dims=feature_dims,  # Feature branch supervision dimension.
            prototype_npz_path=prototype_npz_path,  # Office prototype dictionary path.
            prototype_bridge_path=prototype_bridge_path,  # Use bridge json to avoid object-array npz metadata dependence.
            prototype_field='latent_256',  # Use higher-dimensional PCA prototypes.
            class_names=occ_names,  # Raw Office-79 class order used by GT labels.
            prototype_decode=True,  # Decode semantics by prototype similarity at test time.
            similarity='cosine',  # Nearest-prototype cosine similarity.
            only_positive=True,  # Apply feature supervision only to occupied queries.
            strong_pos_dist_thr=0.10,  # Strong semantic positive distance.
            weak_pos_dist_thr=0.20,  # Weak semantic positive distance.
            weak_pos_weight=0.35,  # Downweight weaker semantic matches instead of dropping them.
            temperature=0.07,  # Prototype classification temperature.
            margin=0.10,  # Hard-negative margin in cosine space.
            tail_feature_weight=2.0,  # Extra tail-class weight for semantic feature learning.
            loss_weights=dict(
                cosine=1.0,  # Keep open-vocabulary prototype alignment as the main signal.
                mse=0.0,  # Disable dense regression-style mse for the mixed-loss run.
                ce=0.5,  # Auxiliary prototype classification term.
                margin=0.25,  # Hard-negative separation term.
            ),
        ),

        # Binary occupancy and regression losses.
        loss_occ=dict(
            type='FocalLoss',
            _scope_='mmdet',
            gamma=2.0,
    ...
    train_cfg=dict(
        pts=dict(
            ...
            occ_target_cfg=dict(
                pos_dist_thr=0.10,  # Positive occupied query distance threshold.
                neg_dist_thr=0.25,  # Negative occupied query distance threshold.
            ),
...
test_pipeline = [
    ...
    dict(
        ...
        sweeps_num=num_frames - 1,
        test_mode=True,
        force_offline=True,  # Keep single-GPU val/test aligned with 9-frame offline training layout.
        imdecode_backend=imdecode_backend,
        cam_types=dataset_cfg['cam_types'],
...
# -------------------- Dataloaders --------------------
batch_size = 16  # Per-GPU train batch size.
