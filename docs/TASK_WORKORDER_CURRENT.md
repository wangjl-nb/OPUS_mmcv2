...
- `configs/opusv1-fusion_nusc-occ3d/TT_Office_mapanything_640x640_9f_100e_tpv_gt-depth_binary-occ_pca256_mixloss_flat.py`: narrowed all stages to side cameras only (`LEFT/BACK/FRONT/RIGHT`), enabled batch-shared dynamic 1-4 view training, and wired GT/eval to `mask_camera_bits` fields when present.
...
- `loaders/tartanground_occ3d_dataset.py`: current frame and camera sweeps now honor configured `dataset_cfg.cam_types`, so current/eval pipelines can permanently drop `TOP/BOTTOM`.
- `loaders/pipelines/loading.py`: `LoadOcc3DFromFile` can optionally rebuild `mask_camera` from `mask_camera_bits/camera_names`; `LoadPointsFromMultiViewDepth` now emits `depth_point_view_ids`; and new `PointsRangeFilterWithViewIds` keeps those ids aligned after point-range filtering.
- `loaders/pipelines/pack_occ3d_inputs.py`: packs `mask_camera_bits` into `data_samples` and preserves GT camera-name metadata.
- `loaders/metrics/occ3d_metric.py`: evaluation can reconstruct camera masks from GT bitfields for a fixed camera subset.
- `models/backbones/mapanything_occ_encoder.py`: added `runtime_num_views` so the external image encoder can truly run on 1-4 active views per frame.
- `models/opusv1_fusion/opus.py`: added batch-shared dynamic view selection, active-view gather/scatter for image features, depth-point filtering by `depth_point_view_ids`, and GT `mask_camera` remasking from `mask_camera_bits`.
...
- `[2026-03-13]` Implemented dynamic side-camera training for the office `pca256_mixloss` config: current/test pipelines now use only `CAM_LEFT/CAM_BACK/CAM_FRONT/CAM_RIGHT`, depth points carry `depth_point_view_ids`, and `OPUSV1Fusion` can batch-share a 1-4 camera subset across all 9 frames while scatter-restoring image features to full TN for the transformer -> done.
- `[2026-03-13]` Added GT/eval compatibility hooks for per-camera visibility: `LoadOcc3DFromFile` and `Occ3DMetric` can rebuild `mask_camera` from `mask_camera_bits + camera_names` using the selected side-camera subset; `PackOcc3DInputs` now carries `mask_camera_bits` into `data_samples` -> done.
- `[2026-03-13]` Dynamic-view model smoke passed on the updated office config with synthetic per-camera GT bitmasks: one-sample `loss()` succeeded for both `keep_count=1` and `keep_count=4`, confirming the new gather/scatter + point filtering + GT remasking path -> done.
- `[2026-03-13]` Re-generated office GT under `data/tartanground_demo/gts_camvisbits/` for `train_with_depth/val_with_depth/test_with_depth` using the office-scene label maps (`/root/wjl/tartanground_demo/Office/seg_label_map.json` and `seg_label.json`) so new files contain `mask_camera_bits/camera_names` while staying in the Office-79 semantic label space -> done.
- `[2026-03-13]` Real-GT dynamic-view smokes passed in `opus-mmcv2`: current office config now parses with `total_epochs=150`, reads `gts_camvisbits`, builds samples with real `mask_camera_bits`, and succeeds on one-sample `loss()` both in direct tensor mode and runner-like `pseudo_collate` input mode -> done.
- `[2026-03-13]` Launched formal 8-GPU training on the updated dynamic-view office config (now `150` epochs, `occ_root=gts_camvisbits`, side cameras only). Active run dir: `outputs/OPUSV1Fusion/TT_Office_mapanything_640x640_9f_100e_tpv_gt-depth_binary-occ_pca256_mixloss_flat_2026-03-13/00-35-12/`; confirmed `Epoch(train)[1][3/27]` reached -> in_progress.
- `[2026-03-13]` Fixed the dynamic-view first-epoch crash: camera-subset sampling in `models/opusv1_fusion/opus.py` now resamples until both filtered depth points and camera-visible occupied GT are non-empty (falling back to all 4 side cameras if needed), and `models/opusv1_fusion/opus_head.py` now skips rare empty-GT samples instead of raising `FloatingPointError` -> done.
- `[2026-03-13]` Verified the fix with a real 8-GPU 1-epoch probe run (`outputs/OPUSV1Fusion/TT_Office_mapanything_640x640_9f_100e_tpv_gt-depth_binary-occ_pca256_mixloss_flat_2026-03-13/10-02-30-fixprobe/`): training passed the previous crash point `Epoch(train)[1][18/27]`, finished `Epoch(train)[1][27/27]`, saved checkpoint, and completed `Epoch(val)[1]` without failure -> done.
- `[2026-03-13]` Relaunched the formal 150-epoch dynamic-view office training after the fix; stdout log: `outputs/analysis/train_dynamic4_office_150e_20260313_100430.stdout.log` -> in_progress.
- `[2026-03-13]` Added runtime logging for dynamic-view training decisions in `models/opusv1_fusion/opus.py`: each train iter now logs `train_view_dropout_keep_count`, `train_view_dropout_attempts`, `train_view_dropout_fallback`, `train_view_dropout_reject_empty_points`, and `train_view_dropout_reject_empty_gt` -> done.
- `[2026-03-13]` Restarted the formal 150-epoch dynamic-view office run to pick up the new logging; current stdout log: `outputs/analysis/train_dynamic4_office_150e_20260313_101930.stdout.log`. Verified first iter logs show varying keep counts (`1`, `3`, `4`) with `fallback=0` and zero rejection counters so far -> in_progress.
- `[2026-03-13]` Added `talk2dino_textsim` visualization mode to `scripts/compare_multiframe_and_query_vis.py`: it now runs `1-view(CAM_FRONT) / 2-view(CAM_FRONT+CAM_RIGHT) / 4-view(side cameras)` exports on the same target sample set and forwards camera-subset overrides into `scripts/export_text_query_similarity_pointcloud.py` -> done.
- `[2026-03-13]` Extended `scripts/export_text_query_similarity_pointcloud.py` with runtime camera-subset config patching, subset `mask_camera_bits` masking, current-frame image copying, and automatic fallback from missing Talk2DINO helper config to training prototype lookup (`prototype_npz + bridge json`) -> done.
- `[2026-03-13]` Fixed multiframe export history semantics in `scripts/inference_demo_multiframe.py`: random/explicit target selection now expands to same-scene previous `history_frames` context before inference instead of fusing only over the sampled subset. Lightweight validation on the office val split with `seed=0` now gives target index `84` and history chain `[79, 80, 81, 82, 83, 84]` (same scene, strict previous-frame chain) -> done.
