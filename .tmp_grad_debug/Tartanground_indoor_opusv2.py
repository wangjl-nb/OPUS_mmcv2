batch_size = 32
bda_aug_conf = dict(flip_dx_ratio=0.5, flip_dy_ratio=0.5)
cls_weights = [
    10,
    2,
    15,
    23,
    7,
    28,
    26,
    14,
    18,
    12,
    7,
    22,
    20,
    30,
    10,
    13,
    17,
    14,
    26,
    27,
    1,
    3,
    5,
    26,
    7,
    18,
    29,
    16,
    9,
    28,
    2,
    9,
    28,
    7,
    20,
    12,
    12,
    27,
    22,
    28,
    14,
    18,
    15,
    26,
    16,
    10,
    14,
    25,
    21,
    13,
    29,
    4,
    8,
    27,
    5,
    28,
    28,
    7,
    15,
    1,
    26,
    26,
    26,
    18,
    21,
    29,
    27,
    25,
    4,
    3,
    21,
    20,
    4,
    20,
    24,
    14,
    29,
    7,
    11,
    12,
    2,
    5,
    9,
    6,
    16,
    11,
    8,
    19,
    9,
    12,
    24,
    23,
    26,
    25,
    17,
    23,
    27,
    21,
    25,
    22,
    16,
    5,
    2,
    17,
    22,
    18,
    27,
    18,
    8,
    12,
    26,
    26,
    28,
    9,
    13,
    14,
    23,
    7,
    6,
    24,
    22,
    12,
    23,
    5,
    16,
    25,
    5,
    7,
    2,
    27,
    8,
    15,
    4,
    16,
    28,
    13,
    5,
    4,
    19,
    9,
    29,
    15,
    23,
    24,
    4,
    16,
    15,
    8,
    21,
    11,
    16,
    4,
    1,
    6,
    20,
    9,
    5,
    15,
    7,
    17,
    23,
    26,
]
custom_imports = dict(
    allow_failed_imports=False, imports=[
        'models',
        'loaders',
    ])
dataset_cfg = dict(
    cam_types=[
        'CAM_LEFT',
        'CAM_BACK',
        'CAM_FRONT',
        'CAM_BOTTOM',
        'CAM_TOP',
        'CAM_RIGHT',
    ],
    class_names=[
        'others',
        'acousticceiling',
        'airconditiongvent',
        'atm',
        'awning',
        'bakerycounter',
        'bannersign',
        'barcounter',
        'barrel',
        'barshelf',
        'beam',
        'bed',
        'bench',
        'bleach',
        'blinds',
        'boatcovered',
        'book',
        'boxedfood',
        'boxedlaundrydetergent',
        'bp',
        'building',
        'buildingtunnel',
        'cabinet',
        'cabinets',
        'cable',
        'cabledrum',
        'cameraactor',
        'car',
        'carpet',
        'cashregister',
        'ceiling',
        'cementcolumn',
        'chain',
        'chair',
        'chairs',
        'chasis',
        'childactor',
        'chipscylinder',
        'cieling',
        'cleaningpowder',
        'column',
        'concretebarricade',
        'couch',
        'counch',
        'cranerail',
        'cucumbers',
        'cupboard',
        'curtain',
        'decorativecookiebox',
        'desk',
        'donuts',
        'door',
        'doorway',
        'dumpster',
        'elevator',
        'elevatordoor',
        'fabricsoftener',
        'fence',
        'fencecolumn',
        'floor',
        'flowerpot',
        'fluorescentlight',
        'foldingscreen',
        'frozenfoodbagged',
        'fryingoil',
        'garagedoor',
        'garbagecan',
        'gate',
        'grass',
        'ground',
        'handtruck',
        'heater',
        'instancedfoliageactor',
        'ivy',
        'laboratorytable',
        'lamp',
        'largeconduit',
        'light',
        'lightbulbstring',
        'meatchub',
        'metalcieling',
        'metalfloor',
        'metalhandrail',
        'metalpanel',
        'metalpillar',
        'metalplatform',
        'metalpole',
        'metalramp',
        'metalstair',
        'metalsupport',
        'metaltop',
        'mirror',
        'monitor',
        'oranges',
        'overheadcrane',
        'painting',
        'papertowels',
        'petfoodtub',
        'piano',
        'picture',
        'pictureframe',
        'pillar',
        'pipe',
        'plant',
        'plantholder',
        'platform',
        'post',
        'railgatebase',
        'railing',
        'railtrack',
        'railtrackend',
        'receptiondesk',
        'refrigerator',
        'road',
        'robotarm',
        'rock',
        'rollingcabinet',
        'roof',
        'rug',
        'saucebottle',
        'screendivider',
        'seat',
        'sewergrate',
        'shampoo',
        'shelf',
        'shoppingbags',
        'sidewalk',
        'sign',
        'sky',
        'sodabottle',
        'sofa',
        'stair',
        'stairs',
        'staticmeshactor',
        'storagerack',
        'streetlight',
        'supportarch',
        'supportbeam',
        'supportcolumn',
        'table',
        'tireassembly',
        'toolbox',
        'trashcan',
        'tree',
        'tunnel',
        'tv',
        'vendingmachine',
        'vent',
        'ventilationgrill',
        'ventpipe',
        'vine',
        'walkway',
        'wall',
        'wallarch',
        'wallarchway',
        'walllight',
        'walls',
        'window',
        'windowframe',
        'woodenbox',
        'woodenpallet',
        'woodenplank',
        'free',
    ],
    empty_label=162,
    num_views=6,
    occ_io=dict(
        mask_camera_key='mask_camera',
        mask_lidar_key='mask_lidar',
        path_template='{scene_name}/{token}/labels.npz',
        semantics_key='semantics'),
    pc_range=[
        -20.0,
        -20.0,
        -3.0,
        20.0,
        20.0,
        5.0,
    ],
    ray=dict(
        lidar=dict(
            azimuth_step_deg=1.0, mode='nuscenes_default', pitch_angles=None),
        max_origins=8,
        num_workers=8,
        origin_xy_bound=39.0),
    voxel_size=[
        0.05,
        0.05,
        0.05,
    ])
dataset_root = '/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/'
dataset_type = 'TartangroundOcc3DDataset'
default_hooks = dict(
    checkpoint=dict(
        interval=10, max_keep_ckpts=5, save_last=True, type='CheckpointHook'),
    logger=dict(interval=1, type='LoggerHook'),
    param_scheduler=dict(type='ParamSchedulerHook'),
    sampler_seed=dict(type='DistSamplerSeedHook'),
    timer=dict(type='IterTimerHook'))
default_scope = 'mmdet3d'
embed_dims = 256
env_cfg = dict(
    cudnn_benchmark=True,
    dist_cfg=dict(backend='nccl'),
    mp_cfg=dict(mp_start_method='fork', opencv_num_threads=0))
ida_aug_conf = dict(
    H=640,
    W=640,
    bot_pct_lim=(
        0.0,
        0.0,
    ),
    final_dim=(
        640,
        640,
    ),
    rand_flip=True,
    resize_lim=(
        1.0,
        1.0,
    ),
    rot_lim=(
        0.0,
        0.0,
    ))
imdecode_backend = 'pillow'
img_backbone = dict(
    _scope_='mmdet',
    depth=50,
    frozen_stages=1,
    norm_cfg=dict(requires_grad=True, type='BN2d'),
    norm_eval=True,
    num_stages=4,
    out_indices=(
        0,
        1,
        2,
        3,
    ),
    style='pytorch',
    type='ResNet',
    with_cp=True)
img_neck = dict(
    _scope_='mmdet',
    in_channels=[
        256,
        512,
        1024,
        2048,
    ],
    num_outs=4,
    out_channels=256,
    type='FPN')
img_norm_cfg = dict(
    mean=[
        123.675,
        116.28,
        103.53,
    ],
    std=[
        58.395,
        57.12,
        57.375,
    ],
    to_rgb=True)
input_modality = dict(
    use_camera=True,
    use_external=True,
    use_lidar=False,
    use_map=False,
    use_radar=False)
launcher = 'none'
load_from = 'pretrain/fusion_pretrain_model.pth'
log_processor = dict(by_epoch=True, type='LogProcessor', window_size=1)
model = dict(
    data_aug=dict(
        img_color_aug=True,
        img_norm_cfg=dict(
            mean=[
                123.675,
                116.28,
                103.53,
            ],
            std=[
                58.395,
                57.12,
                57.375,
            ],
            to_rgb=True),
        img_pad_cfg=dict(size_divisor=32)),
    data_preprocessor=dict(type='BaseDataPreprocessor'),
    img_backbone=dict(
        _scope_='mmdet',
        depth=50,
        frozen_stages=1,
        norm_cfg=dict(requires_grad=True, type='BN2d'),
        norm_eval=True,
        num_stages=4,
        out_indices=(
            0,
            1,
            2,
            3,
        ),
        style='pytorch',
        type='ResNet',
        with_cp=True),
    img_neck=dict(
        _scope_='mmdet',
        in_channels=[
            256,
            512,
            1024,
            2048,
        ],
        num_outs=4,
        out_channels=256,
        type='FPN'),
    pts_bbox_head=dict(
        empty_label=162,
        in_channels=256,
        loss_cls=dict(
            _scope_='mmdet',
            alpha=0.25,
            gamma=2.0,
            loss_weight=2.0,
            type='FocalLoss',
            use_sigmoid=True),
        loss_pts=dict(
            _scope_='mmdet', beta=0.2, loss_weight=0.5, type='SmoothL1Loss'),
        num_classes=162,
        num_query=6400,
        pc_range=[
            -20.0,
            -20.0,
            -3.0,
            20.0,
            20.0,
            5.0,
        ],
        transformer=dict(
            embed_dims=256,
            num_frames=9,
            num_layers=5,
            num_levels=4,
            num_points=4,
            num_pt_channels=32,
            num_refines=[
                1,
                4,
                4,
                16,
                32,
            ],
            num_views=6,
            pc_range=[
                -20.0,
                -20.0,
                -3.0,
                20.0,
                20.0,
                5.0,
            ],
            scales=[
                0.5,
            ],
            type='OPUSV2Transformer'),
        type='OPUSV2Head',
        voxel_size=[
            0.05,
            0.05,
            0.05,
        ]),
    stop_prev_grad=0,
    test_cfg=dict(pts=dict(score_thr=0.3)),
    train_cfg=dict(
        pts=dict(
            cls_weights=[
                10,
                2,
                15,
                23,
                7,
                28,
                26,
                14,
                18,
                12,
                7,
                22,
                20,
                30,
                10,
                13,
                17,
                14,
                26,
                27,
                1,
                3,
                5,
                26,
                7,
                18,
                29,
                16,
                9,
                28,
                2,
                9,
                28,
                7,
                20,
                12,
                12,
                27,
                22,
                28,
                14,
                18,
                15,
                26,
                16,
                10,
                14,
                25,
                21,
                13,
                29,
                4,
                8,
                27,
                5,
                28,
                28,
                7,
                15,
                1,
                26,
                26,
                26,
                18,
                21,
                29,
                27,
                25,
                4,
                3,
                21,
                20,
                4,
                20,
                24,
                14,
                29,
                7,
                11,
                12,
                2,
                5,
                9,
                6,
                16,
                11,
                8,
                19,
                9,
                12,
                24,
                23,
                26,
                25,
                17,
                23,
                27,
                21,
                25,
                22,
                16,
                5,
                2,
                17,
                22,
                18,
                27,
                18,
                8,
                12,
                26,
                26,
                28,
                9,
                13,
                14,
                23,
                7,
                6,
                24,
                22,
                12,
                23,
                5,
                16,
                25,
                5,
                7,
                2,
                27,
                8,
                15,
                4,
                16,
                28,
                13,
                5,
                4,
                19,
                9,
                29,
                15,
                23,
                24,
                4,
                16,
                15,
                8,
                21,
                11,
                16,
                4,
                1,
                6,
                20,
                9,
                5,
                15,
                7,
                17,
                23,
                26,
            ],
            rare_classes=[
                13,
                26,
                50,
                65,
                76,
                140,
                134,
                112,
                56,
                39,
                32,
                5,
                29,
                55,
                19,
                96,
                37,
                66,
                106,
                129,
                53,
                61,
                23,
                92,
            ],
            rare_weights=2,
            safe_injection=True,
            safe_knn_chunk_size=50000,
            safe_max_gt_points=60000,
            safe_prioritize_visible=True)),
    type='OPUSV2',
    use_grid_mask=False)
num_frames = 9
num_layers = 5
num_levels = 4
num_points = 4
num_pt_channels = 32
num_query = 6400
num_refines = [
    1,
    4,
    4,
    16,
    32,
]
object_names = [
    'car',
    'truck',
    'construction_vehicle',
    'bus',
    'trailer',
    'barrier',
    'motorcycle',
    'bicycle',
    'pedestrian',
    'traffic_cone',
]
occ_eval_names = [
    'others',
    'acousticceiling',
    'airconditiongvent',
    'atm',
    'awning',
    'bakerycounter',
    'bannersign',
    'barcounter',
    'barrel',
    'barshelf',
    'beam',
    'bed',
    'bench',
    'bleach',
    'blinds',
    'boatcovered',
    'book',
    'boxedfood',
    'boxedlaundrydetergent',
    'bp',
    'building',
    'buildingtunnel',
    'cabinet',
    'cabinets',
    'cable',
    'cabledrum',
    'cameraactor',
    'car',
    'carpet',
    'cashregister',
    'ceiling',
    'cementcolumn',
    'chain',
    'chair',
    'chairs',
    'chasis',
    'childactor',
    'chipscylinder',
    'cieling',
    'cleaningpowder',
    'column',
    'concretebarricade',
    'couch',
    'counch',
    'cranerail',
    'cucumbers',
    'cupboard',
    'curtain',
    'decorativecookiebox',
    'desk',
    'donuts',
    'door',
    'doorway',
    'dumpster',
    'elevator',
    'elevatordoor',
    'fabricsoftener',
    'fence',
    'fencecolumn',
    'floor',
    'flowerpot',
    'fluorescentlight',
    'foldingscreen',
    'frozenfoodbagged',
    'fryingoil',
    'garagedoor',
    'garbagecan',
    'gate',
    'grass',
    'ground',
    'handtruck',
    'heater',
    'instancedfoliageactor',
    'ivy',
    'laboratorytable',
    'lamp',
    'largeconduit',
    'light',
    'lightbulbstring',
    'meatchub',
    'metalcieling',
    'metalfloor',
    'metalhandrail',
    'metalpanel',
    'metalpillar',
    'metalplatform',
    'metalpole',
    'metalramp',
    'metalstair',
    'metalsupport',
    'metaltop',
    'mirror',
    'monitor',
    'oranges',
    'overheadcrane',
    'painting',
    'papertowels',
    'petfoodtub',
    'piano',
    'picture',
    'pictureframe',
    'pillar',
    'pipe',
    'plant',
    'plantholder',
    'platform',
    'post',
    'railgatebase',
    'railing',
    'railtrack',
    'railtrackend',
    'receptiondesk',
    'refrigerator',
    'road',
    'robotarm',
    'rock',
    'rollingcabinet',
    'roof',
    'rug',
    'saucebottle',
    'screendivider',
    'seat',
    'sewergrate',
    'shampoo',
    'shelf',
    'shoppingbags',
    'sidewalk',
    'sign',
    'sky',
    'sodabottle',
    'sofa',
    'stair',
    'stairs',
    'staticmeshactor',
    'storagerack',
    'streetlight',
    'supportarch',
    'supportbeam',
    'supportcolumn',
    'table',
    'tireassembly',
    'toolbox',
    'trashcan',
    'tree',
    'tunnel',
    'tv',
    'vendingmachine',
    'vent',
    'ventilationgrill',
    'ventpipe',
    'vine',
    'walkway',
    'wall',
    'wallarch',
    'wallarchway',
    'walllight',
    'walls',
    'window',
    'windowframe',
    'woodenbox',
    'woodenpallet',
    'woodenplank',
    'free',
]
occ_names = [
    'others',
    'acousticceiling',
    'airconditiongvent',
    'atm',
    'awning',
    'bakerycounter',
    'bannersign',
    'barcounter',
    'barrel',
    'barshelf',
    'beam',
    'bed',
    'bench',
    'bleach',
    'blinds',
    'boatcovered',
    'book',
    'boxedfood',
    'boxedlaundrydetergent',
    'bp',
    'building',
    'buildingtunnel',
    'cabinet',
    'cabinets',
    'cable',
    'cabledrum',
    'cameraactor',
    'car',
    'carpet',
    'cashregister',
    'ceiling',
    'cementcolumn',
    'chain',
    'chair',
    'chairs',
    'chasis',
    'childactor',
    'chipscylinder',
    'cieling',
    'cleaningpowder',
    'column',
    'concretebarricade',
    'couch',
    'counch',
    'cranerail',
    'cucumbers',
    'cupboard',
    'curtain',
    'decorativecookiebox',
    'desk',
    'donuts',
    'door',
    'doorway',
    'dumpster',
    'elevator',
    'elevatordoor',
    'fabricsoftener',
    'fence',
    'fencecolumn',
    'floor',
    'flowerpot',
    'fluorescentlight',
    'foldingscreen',
    'frozenfoodbagged',
    'fryingoil',
    'garagedoor',
    'garbagecan',
    'gate',
    'grass',
    'ground',
    'handtruck',
    'heater',
    'instancedfoliageactor',
    'ivy',
    'laboratorytable',
    'lamp',
    'largeconduit',
    'light',
    'lightbulbstring',
    'meatchub',
    'metalcieling',
    'metalfloor',
    'metalhandrail',
    'metalpanel',
    'metalpillar',
    'metalplatform',
    'metalpole',
    'metalramp',
    'metalstair',
    'metalsupport',
    'metaltop',
    'mirror',
    'monitor',
    'oranges',
    'overheadcrane',
    'painting',
    'papertowels',
    'petfoodtub',
    'piano',
    'picture',
    'pictureframe',
    'pillar',
    'pipe',
    'plant',
    'plantholder',
    'platform',
    'post',
    'railgatebase',
    'railing',
    'railtrack',
    'railtrackend',
    'receptiondesk',
    'refrigerator',
    'road',
    'robotarm',
    'rock',
    'rollingcabinet',
    'roof',
    'rug',
    'saucebottle',
    'screendivider',
    'seat',
    'sewergrate',
    'shampoo',
    'shelf',
    'shoppingbags',
    'sidewalk',
    'sign',
    'sky',
    'sodabottle',
    'sofa',
    'stair',
    'stairs',
    'staticmeshactor',
    'storagerack',
    'streetlight',
    'supportarch',
    'supportbeam',
    'supportcolumn',
    'table',
    'tireassembly',
    'toolbox',
    'trashcan',
    'tree',
    'tunnel',
    'tv',
    'vendingmachine',
    'vent',
    'ventilationgrill',
    'ventpipe',
    'vine',
    'walkway',
    'wall',
    'wallarch',
    'wallarchway',
    'walllight',
    'walls',
    'window',
    'windowframe',
    'woodenbox',
    'woodenpallet',
    'woodenplank',
]
occ_root = '/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/'
optim_wrapper = dict(
    clip_grad=dict(max_norm=35, norm_type=2),
    loss_scale=512.0,
    optimizer=dict(lr=0.0002, type='AdamW', weight_decay=0.01),
    paramwise_cfg=dict(
        custom_keys=dict(
            img_backbone=dict(lr_mult=0.1), sampling_offset=dict(
                lr_mult=0.1))),
    type='AmpOptimWrapper')
optimizer = dict(lr=0.0002, type='AdamW', weight_decay=0.01)
param_scheduler = [
    dict(
        begin=0,
        by_epoch=False,
        end=500,
        start_factor=0.3333333333333333,
        type='LinearLR'),
    dict(
        T_max=100,
        begin=0,
        by_epoch=True,
        end=100,
        eta_min=2.0000000000000002e-07,
        type='CosineAnnealingLR'),
]
point_cloud_range = [
    -20.0,
    -20.0,
    -3.0,
    20.0,
    20.0,
    5.0,
]
randomness = dict(deterministic=False, seed=0)
rare_classes = [
    13,
    26,
    50,
    65,
    76,
    140,
    134,
    112,
    56,
    39,
    32,
    5,
    29,
    55,
    19,
    96,
    37,
    66,
    106,
    129,
    53,
    61,
    23,
    92,
]
resume_from = None
test_cfg = dict(type='TestLoop')
test_dataloader = dict(
    batch_size=1,
    collate_fn=dict(type='pseudo_collate'),
    dataset=dict(
        ann_file='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/test.pkl',
        classes=[
            'car',
            'truck',
            'construction_vehicle',
            'bus',
            'trailer',
            'barrier',
            'motorcycle',
            'bicycle',
            'pedestrian',
            'traffic_cone',
        ],
        data_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/',
        dataset_cfg=dict(
            cam_types=[
                'CAM_LEFT',
                'CAM_BACK',
                'CAM_FRONT',
                'CAM_BOTTOM',
                'CAM_TOP',
                'CAM_RIGHT',
            ],
            class_names=[
                'others',
                'acousticceiling',
                'airconditiongvent',
                'atm',
                'awning',
                'bakerycounter',
                'bannersign',
                'barcounter',
                'barrel',
                'barshelf',
                'beam',
                'bed',
                'bench',
                'bleach',
                'blinds',
                'boatcovered',
                'book',
                'boxedfood',
                'boxedlaundrydetergent',
                'bp',
                'building',
                'buildingtunnel',
                'cabinet',
                'cabinets',
                'cable',
                'cabledrum',
                'cameraactor',
                'car',
                'carpet',
                'cashregister',
                'ceiling',
                'cementcolumn',
                'chain',
                'chair',
                'chairs',
                'chasis',
                'childactor',
                'chipscylinder',
                'cieling',
                'cleaningpowder',
                'column',
                'concretebarricade',
                'couch',
                'counch',
                'cranerail',
                'cucumbers',
                'cupboard',
                'curtain',
                'decorativecookiebox',
                'desk',
                'donuts',
                'door',
                'doorway',
                'dumpster',
                'elevator',
                'elevatordoor',
                'fabricsoftener',
                'fence',
                'fencecolumn',
                'floor',
                'flowerpot',
                'fluorescentlight',
                'foldingscreen',
                'frozenfoodbagged',
                'fryingoil',
                'garagedoor',
                'garbagecan',
                'gate',
                'grass',
                'ground',
                'handtruck',
                'heater',
                'instancedfoliageactor',
                'ivy',
                'laboratorytable',
                'lamp',
                'largeconduit',
                'light',
                'lightbulbstring',
                'meatchub',
                'metalcieling',
                'metalfloor',
                'metalhandrail',
                'metalpanel',
                'metalpillar',
                'metalplatform',
                'metalpole',
                'metalramp',
                'metalstair',
                'metalsupport',
                'metaltop',
                'mirror',
                'monitor',
                'oranges',
                'overheadcrane',
                'painting',
                'papertowels',
                'petfoodtub',
                'piano',
                'picture',
                'pictureframe',
                'pillar',
                'pipe',
                'plant',
                'plantholder',
                'platform',
                'post',
                'railgatebase',
                'railing',
                'railtrack',
                'railtrackend',
                'receptiondesk',
                'refrigerator',
                'road',
                'robotarm',
                'rock',
                'rollingcabinet',
                'roof',
                'rug',
                'saucebottle',
                'screendivider',
                'seat',
                'sewergrate',
                'shampoo',
                'shelf',
                'shoppingbags',
                'sidewalk',
                'sign',
                'sky',
                'sodabottle',
                'sofa',
                'stair',
                'stairs',
                'staticmeshactor',
                'storagerack',
                'streetlight',
                'supportarch',
                'supportbeam',
                'supportcolumn',
                'table',
                'tireassembly',
                'toolbox',
                'trashcan',
                'tree',
                'tunnel',
                'tv',
                'vendingmachine',
                'vent',
                'ventilationgrill',
                'ventpipe',
                'vine',
                'walkway',
                'wall',
                'wallarch',
                'wallarchway',
                'walllight',
                'walls',
                'window',
                'windowframe',
                'woodenbox',
                'woodenpallet',
                'woodenplank',
                'free',
            ],
            empty_label=162,
            num_views=6,
            occ_io=dict(
                mask_camera_key='mask_camera',
                mask_lidar_key='mask_lidar',
                path_template='{scene_name}/{token}/labels.npz',
                semantics_key='semantics'),
            pc_range=[
                -20.0,
                -20.0,
                -3.0,
                20.0,
                20.0,
                5.0,
            ],
            ray=dict(
                lidar=dict(
                    azimuth_step_deg=1.0,
                    mode='nuscenes_default',
                    pitch_angles=None),
                max_origins=8,
                num_workers=8,
                origin_xy_bound=39.0),
            voxel_size=[
                0.05,
                0.05,
                0.05,
            ]),
        modality=dict(
            use_camera=True,
            use_external=True,
            use_lidar=False,
            use_map=False,
            use_radar=False),
        occ_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/',
        pipeline=[
            dict(
                color_type='color',
                imdecode_backend='pillow',
                to_float32=False,
                type='LoadMultiViewImageFromFiles'),
            dict(
                cam_types=[
                    'CAM_LEFT',
                    'CAM_BACK',
                    'CAM_FRONT',
                    'CAM_BOTTOM',
                    'CAM_TOP',
                    'CAM_RIGHT',
                ],
                imdecode_backend='pillow',
                sweeps_num=8,
                test_mode=True,
                type='LoadMultiViewImageFromMultiSweeps'),
            dict(
                ida_aug_conf=dict(
                    H=640,
                    W=640,
                    bot_pct_lim=(
                        0.0,
                        0.0,
                    ),
                    final_dim=(
                        640,
                        640,
                    ),
                    rand_flip=True,
                    resize_lim=(
                        1.0,
                        1.0,
                    ),
                    rot_lim=(
                        0.0,
                        0.0,
                    )),
                training=False,
                type='RandomTransformImage'),
            dict(
                meta_keys=(
                    'sample_idx',
                    'sample_token',
                    'scene_name',
                    'filename',
                    'ori_shape',
                    'img_shape',
                    'pad_shape',
                    'ego2occ',
                    'ego2img',
                    'ego2lidar',
                    'img_timestamp',
                ),
                type='PackOcc3DInputs'),
        ],
        test_mode=True,
        type='TartangroundOcc3DDataset'),
    num_workers=0,
    persistent_workers=False,
    sampler=dict(shuffle=False, type='DefaultSampler'))
test_evaluator = dict(
    ann_file='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/test.pkl',
    class_names=[
        'others',
        'acousticceiling',
        'airconditiongvent',
        'atm',
        'awning',
        'bakerycounter',
        'bannersign',
        'barcounter',
        'barrel',
        'barshelf',
        'beam',
        'bed',
        'bench',
        'bleach',
        'blinds',
        'boatcovered',
        'book',
        'boxedfood',
        'boxedlaundrydetergent',
        'bp',
        'building',
        'buildingtunnel',
        'cabinet',
        'cabinets',
        'cable',
        'cabledrum',
        'cameraactor',
        'car',
        'carpet',
        'cashregister',
        'ceiling',
        'cementcolumn',
        'chain',
        'chair',
        'chairs',
        'chasis',
        'childactor',
        'chipscylinder',
        'cieling',
        'cleaningpowder',
        'column',
        'concretebarricade',
        'couch',
        'counch',
        'cranerail',
        'cucumbers',
        'cupboard',
        'curtain',
        'decorativecookiebox',
        'desk',
        'donuts',
        'door',
        'doorway',
        'dumpster',
        'elevator',
        'elevatordoor',
        'fabricsoftener',
        'fence',
        'fencecolumn',
        'floor',
        'flowerpot',
        'fluorescentlight',
        'foldingscreen',
        'frozenfoodbagged',
        'fryingoil',
        'garagedoor',
        'garbagecan',
        'gate',
        'grass',
        'ground',
        'handtruck',
        'heater',
        'instancedfoliageactor',
        'ivy',
        'laboratorytable',
        'lamp',
        'largeconduit',
        'light',
        'lightbulbstring',
        'meatchub',
        'metalcieling',
        'metalfloor',
        'metalhandrail',
        'metalpanel',
        'metalpillar',
        'metalplatform',
        'metalpole',
        'metalramp',
        'metalstair',
        'metalsupport',
        'metaltop',
        'mirror',
        'monitor',
        'oranges',
        'overheadcrane',
        'painting',
        'papertowels',
        'petfoodtub',
        'piano',
        'picture',
        'pictureframe',
        'pillar',
        'pipe',
        'plant',
        'plantholder',
        'platform',
        'post',
        'railgatebase',
        'railing',
        'railtrack',
        'railtrackend',
        'receptiondesk',
        'refrigerator',
        'road',
        'robotarm',
        'rock',
        'rollingcabinet',
        'roof',
        'rug',
        'saucebottle',
        'screendivider',
        'seat',
        'sewergrate',
        'shampoo',
        'shelf',
        'shoppingbags',
        'sidewalk',
        'sign',
        'sky',
        'sodabottle',
        'sofa',
        'stair',
        'stairs',
        'staticmeshactor',
        'storagerack',
        'streetlight',
        'supportarch',
        'supportbeam',
        'supportcolumn',
        'table',
        'tireassembly',
        'toolbox',
        'trashcan',
        'tree',
        'tunnel',
        'tv',
        'vendingmachine',
        'vent',
        'ventilationgrill',
        'ventpipe',
        'vine',
        'walkway',
        'wall',
        'wallarch',
        'wallarchway',
        'walllight',
        'walls',
        'window',
        'windowframe',
        'woodenbox',
        'woodenpallet',
        'woodenplank',
        'free',
    ],
    empty_label=162,
    focus_eval=dict(
        ema_momentum=0.9,
        enabled=True,
        fallback='rare_classes',
        freq_thr=0.02,
        max_classes=24,
        min_classes=8,
        policy='ema_freq',
        rare_classes=[
            13,
            26,
            50,
            65,
            76,
            140,
            134,
            112,
            56,
            39,
            32,
            5,
            29,
            55,
            19,
            96,
            37,
            66,
            106,
            129,
            53,
            61,
            23,
            92,
        ]),
    mask_camera_key='mask_camera',
    mask_lidar_key='mask_lidar',
    miou_num_workers=32,
    occ_path_template='{scene_name}/{token}/labels.npz',
    occ_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/',
    pc_range=[
        -20.0,
        -20.0,
        -3.0,
        20.0,
        20.0,
        5.0,
    ],
    semantics_key='semantics',
    type='Occ3DMetric',
    use_camera_mask=True,
    voxel_size=[
        0.05,
        0.05,
        0.05,
    ])
test_pipeline = [
    dict(
        color_type='color',
        imdecode_backend='pillow',
        to_float32=False,
        type='LoadMultiViewImageFromFiles'),
    dict(
        cam_types=[
            'CAM_LEFT',
            'CAM_BACK',
            'CAM_FRONT',
            'CAM_BOTTOM',
            'CAM_TOP',
            'CAM_RIGHT',
        ],
        imdecode_backend='pillow',
        sweeps_num=8,
        test_mode=True,
        type='LoadMultiViewImageFromMultiSweeps'),
    dict(
        ida_aug_conf=dict(
            H=640,
            W=640,
            bot_pct_lim=(
                0.0,
                0.0,
            ),
            final_dim=(
                640,
                640,
            ),
            rand_flip=True,
            resize_lim=(
                1.0,
                1.0,
            ),
            rot_lim=(
                0.0,
                0.0,
            )),
        training=False,
        type='RandomTransformImage'),
    dict(
        meta_keys=(
            'sample_idx',
            'sample_token',
            'scene_name',
            'filename',
            'ori_shape',
            'img_shape',
            'pad_shape',
            'ego2occ',
            'ego2img',
            'ego2lidar',
            'img_timestamp',
        ),
        type='PackOcc3DInputs'),
]
total_epochs = 100
train_cfg = dict(max_epochs=100, type='EpochBasedTrainLoop', val_interval=10)
train_dataloader = dict(
    batch_size=1,
    collate_fn=dict(type='pseudo_collate'),
    dataset=dict(
        ann_file='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/train.pkl',
        classes=[
            'car',
            'truck',
            'construction_vehicle',
            'bus',
            'trailer',
            'barrier',
            'motorcycle',
            'bicycle',
            'pedestrian',
            'traffic_cone',
        ],
        data_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/',
        dataset_cfg=dict(
            cam_types=[
                'CAM_LEFT',
                'CAM_BACK',
                'CAM_FRONT',
                'CAM_BOTTOM',
                'CAM_TOP',
                'CAM_RIGHT',
            ],
            class_names=[
                'others',
                'acousticceiling',
                'airconditiongvent',
                'atm',
                'awning',
                'bakerycounter',
                'bannersign',
                'barcounter',
                'barrel',
                'barshelf',
                'beam',
                'bed',
                'bench',
                'bleach',
                'blinds',
                'boatcovered',
                'book',
                'boxedfood',
                'boxedlaundrydetergent',
                'bp',
                'building',
                'buildingtunnel',
                'cabinet',
                'cabinets',
                'cable',
                'cabledrum',
                'cameraactor',
                'car',
                'carpet',
                'cashregister',
                'ceiling',
                'cementcolumn',
                'chain',
                'chair',
                'chairs',
                'chasis',
                'childactor',
                'chipscylinder',
                'cieling',
                'cleaningpowder',
                'column',
                'concretebarricade',
                'couch',
                'counch',
                'cranerail',
                'cucumbers',
                'cupboard',
                'curtain',
                'decorativecookiebox',
                'desk',
                'donuts',
                'door',
                'doorway',
                'dumpster',
                'elevator',
                'elevatordoor',
                'fabricsoftener',
                'fence',
                'fencecolumn',
                'floor',
                'flowerpot',
                'fluorescentlight',
                'foldingscreen',
                'frozenfoodbagged',
                'fryingoil',
                'garagedoor',
                'garbagecan',
                'gate',
                'grass',
                'ground',
                'handtruck',
                'heater',
                'instancedfoliageactor',
                'ivy',
                'laboratorytable',
                'lamp',
                'largeconduit',
                'light',
                'lightbulbstring',
                'meatchub',
                'metalcieling',
                'metalfloor',
                'metalhandrail',
                'metalpanel',
                'metalpillar',
                'metalplatform',
                'metalpole',
                'metalramp',
                'metalstair',
                'metalsupport',
                'metaltop',
                'mirror',
                'monitor',
                'oranges',
                'overheadcrane',
                'painting',
                'papertowels',
                'petfoodtub',
                'piano',
                'picture',
                'pictureframe',
                'pillar',
                'pipe',
                'plant',
                'plantholder',
                'platform',
                'post',
                'railgatebase',
                'railing',
                'railtrack',
                'railtrackend',
                'receptiondesk',
                'refrigerator',
                'road',
                'robotarm',
                'rock',
                'rollingcabinet',
                'roof',
                'rug',
                'saucebottle',
                'screendivider',
                'seat',
                'sewergrate',
                'shampoo',
                'shelf',
                'shoppingbags',
                'sidewalk',
                'sign',
                'sky',
                'sodabottle',
                'sofa',
                'stair',
                'stairs',
                'staticmeshactor',
                'storagerack',
                'streetlight',
                'supportarch',
                'supportbeam',
                'supportcolumn',
                'table',
                'tireassembly',
                'toolbox',
                'trashcan',
                'tree',
                'tunnel',
                'tv',
                'vendingmachine',
                'vent',
                'ventilationgrill',
                'ventpipe',
                'vine',
                'walkway',
                'wall',
                'wallarch',
                'wallarchway',
                'walllight',
                'walls',
                'window',
                'windowframe',
                'woodenbox',
                'woodenpallet',
                'woodenplank',
                'free',
            ],
            empty_label=162,
            num_views=6,
            occ_io=dict(
                mask_camera_key='mask_camera',
                mask_lidar_key='mask_lidar',
                path_template='{scene_name}/{token}/labels.npz',
                semantics_key='semantics'),
            pc_range=[
                -20.0,
                -20.0,
                -3.0,
                20.0,
                20.0,
                5.0,
            ],
            ray=dict(
                lidar=dict(
                    azimuth_step_deg=1.0,
                    mode='nuscenes_default',
                    pitch_angles=None),
                max_origins=8,
                num_workers=8,
                origin_xy_bound=39.0),
            voxel_size=[
                0.05,
                0.05,
                0.05,
            ]),
        modality=dict(
            use_camera=True,
            use_external=True,
            use_lidar=False,
            use_map=False,
            use_radar=False),
        occ_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/',
        pipeline=[
            dict(
                color_type='color',
                imdecode_backend='pillow',
                to_float32=False,
                type='LoadMultiViewImageFromFiles'),
            dict(
                cam_types=[
                    'CAM_LEFT',
                    'CAM_BACK',
                    'CAM_FRONT',
                    'CAM_BOTTOM',
                    'CAM_TOP',
                    'CAM_RIGHT',
                ],
                imdecode_backend='pillow',
                sweeps_num=8,
                type='LoadMultiViewImageFromMultiSweeps'),
            dict(
                class_names=[
                    'others',
                    'acousticceiling',
                    'airconditiongvent',
                    'atm',
                    'awning',
                    'bakerycounter',
                    'bannersign',
                    'barcounter',
                    'barrel',
                    'barshelf',
                    'beam',
                    'bed',
                    'bench',
                    'bleach',
                    'blinds',
                    'boatcovered',
                    'book',
                    'boxedfood',
                    'boxedlaundrydetergent',
                    'bp',
                    'building',
                    'buildingtunnel',
                    'cabinet',
                    'cabinets',
                    'cable',
                    'cabledrum',
                    'cameraactor',
                    'car',
                    'carpet',
                    'cashregister',
                    'ceiling',
                    'cementcolumn',
                    'chain',
                    'chair',
                    'chairs',
                    'chasis',
                    'childactor',
                    'chipscylinder',
                    'cieling',
                    'cleaningpowder',
                    'column',
                    'concretebarricade',
                    'couch',
                    'counch',
                    'cranerail',
                    'cucumbers',
                    'cupboard',
                    'curtain',
                    'decorativecookiebox',
                    'desk',
                    'donuts',
                    'door',
                    'doorway',
                    'dumpster',
                    'elevator',
                    'elevatordoor',
                    'fabricsoftener',
                    'fence',
                    'fencecolumn',
                    'floor',
                    'flowerpot',
                    'fluorescentlight',
                    'foldingscreen',
                    'frozenfoodbagged',
                    'fryingoil',
                    'garagedoor',
                    'garbagecan',
                    'gate',
                    'grass',
                    'ground',
                    'handtruck',
                    'heater',
                    'instancedfoliageactor',
                    'ivy',
                    'laboratorytable',
                    'lamp',
                    'largeconduit',
                    'light',
                    'lightbulbstring',
                    'meatchub',
                    'metalcieling',
                    'metalfloor',
                    'metalhandrail',
                    'metalpanel',
                    'metalpillar',
                    'metalplatform',
                    'metalpole',
                    'metalramp',
                    'metalstair',
                    'metalsupport',
                    'metaltop',
                    'mirror',
                    'monitor',
                    'oranges',
                    'overheadcrane',
                    'painting',
                    'papertowels',
                    'petfoodtub',
                    'piano',
                    'picture',
                    'pictureframe',
                    'pillar',
                    'pipe',
                    'plant',
                    'plantholder',
                    'platform',
                    'post',
                    'railgatebase',
                    'railing',
                    'railtrack',
                    'railtrackend',
                    'receptiondesk',
                    'refrigerator',
                    'road',
                    'robotarm',
                    'rock',
                    'rollingcabinet',
                    'roof',
                    'rug',
                    'saucebottle',
                    'screendivider',
                    'seat',
                    'sewergrate',
                    'shampoo',
                    'shelf',
                    'shoppingbags',
                    'sidewalk',
                    'sign',
                    'sky',
                    'sodabottle',
                    'sofa',
                    'stair',
                    'stairs',
                    'staticmeshactor',
                    'storagerack',
                    'streetlight',
                    'supportarch',
                    'supportbeam',
                    'supportcolumn',
                    'table',
                    'tireassembly',
                    'toolbox',
                    'trashcan',
                    'tree',
                    'tunnel',
                    'tv',
                    'vendingmachine',
                    'vent',
                    'ventilationgrill',
                    'ventpipe',
                    'vine',
                    'walkway',
                    'wall',
                    'wallarch',
                    'wallarchway',
                    'walllight',
                    'walls',
                    'window',
                    'windowframe',
                    'woodenbox',
                    'woodenpallet',
                    'woodenplank',
                    'free',
                ],
                empty_label=162,
                mask_camera_key='mask_camera',
                mask_lidar_key='mask_lidar',
                occ_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/',
                path_template='{scene_name}/{token}/labels.npz',
                semantics_key='semantics',
                type='LoadOcc3DFromFile'),
            dict(
                ida_aug_conf=dict(
                    H=640,
                    W=640,
                    bot_pct_lim=(
                        0.0,
                        0.0,
                    ),
                    final_dim=(
                        640,
                        640,
                    ),
                    rand_flip=True,
                    resize_lim=(
                        1.0,
                        1.0,
                    ),
                    rot_lim=(
                        0.0,
                        0.0,
                    )),
                training=True,
                type='RandomTransformImage'),
            dict(
                bda_aug_conf=dict(flip_dx_ratio=0.5, flip_dy_ratio=0.5),
                empty_label=162,
                type='RandomTransformOcc'),
            dict(
                meta_keys=(
                    'sample_idx',
                    'sample_token',
                    'scene_name',
                    'filename',
                    'ori_shape',
                    'img_shape',
                    'pad_shape',
                    'ego2occ',
                    'ego2img',
                    'ego2lidar',
                    'img_timestamp',
                ),
                type='PackOcc3DInputs'),
        ],
        test_mode=False,
        type='TartangroundOcc3DDataset'),
    num_workers=0,
    persistent_workers=False,
    sampler=dict(shuffle=True, type='DefaultSampler'))
train_pipeline = [
    dict(
        color_type='color',
        imdecode_backend='pillow',
        to_float32=False,
        type='LoadMultiViewImageFromFiles'),
    dict(
        cam_types=[
            'CAM_LEFT',
            'CAM_BACK',
            'CAM_FRONT',
            'CAM_BOTTOM',
            'CAM_TOP',
            'CAM_RIGHT',
        ],
        imdecode_backend='pillow',
        sweeps_num=8,
        type='LoadMultiViewImageFromMultiSweeps'),
    dict(
        class_names=[
            'others',
            'acousticceiling',
            'airconditiongvent',
            'atm',
            'awning',
            'bakerycounter',
            'bannersign',
            'barcounter',
            'barrel',
            'barshelf',
            'beam',
            'bed',
            'bench',
            'bleach',
            'blinds',
            'boatcovered',
            'book',
            'boxedfood',
            'boxedlaundrydetergent',
            'bp',
            'building',
            'buildingtunnel',
            'cabinet',
            'cabinets',
            'cable',
            'cabledrum',
            'cameraactor',
            'car',
            'carpet',
            'cashregister',
            'ceiling',
            'cementcolumn',
            'chain',
            'chair',
            'chairs',
            'chasis',
            'childactor',
            'chipscylinder',
            'cieling',
            'cleaningpowder',
            'column',
            'concretebarricade',
            'couch',
            'counch',
            'cranerail',
            'cucumbers',
            'cupboard',
            'curtain',
            'decorativecookiebox',
            'desk',
            'donuts',
            'door',
            'doorway',
            'dumpster',
            'elevator',
            'elevatordoor',
            'fabricsoftener',
            'fence',
            'fencecolumn',
            'floor',
            'flowerpot',
            'fluorescentlight',
            'foldingscreen',
            'frozenfoodbagged',
            'fryingoil',
            'garagedoor',
            'garbagecan',
            'gate',
            'grass',
            'ground',
            'handtruck',
            'heater',
            'instancedfoliageactor',
            'ivy',
            'laboratorytable',
            'lamp',
            'largeconduit',
            'light',
            'lightbulbstring',
            'meatchub',
            'metalcieling',
            'metalfloor',
            'metalhandrail',
            'metalpanel',
            'metalpillar',
            'metalplatform',
            'metalpole',
            'metalramp',
            'metalstair',
            'metalsupport',
            'metaltop',
            'mirror',
            'monitor',
            'oranges',
            'overheadcrane',
            'painting',
            'papertowels',
            'petfoodtub',
            'piano',
            'picture',
            'pictureframe',
            'pillar',
            'pipe',
            'plant',
            'plantholder',
            'platform',
            'post',
            'railgatebase',
            'railing',
            'railtrack',
            'railtrackend',
            'receptiondesk',
            'refrigerator',
            'road',
            'robotarm',
            'rock',
            'rollingcabinet',
            'roof',
            'rug',
            'saucebottle',
            'screendivider',
            'seat',
            'sewergrate',
            'shampoo',
            'shelf',
            'shoppingbags',
            'sidewalk',
            'sign',
            'sky',
            'sodabottle',
            'sofa',
            'stair',
            'stairs',
            'staticmeshactor',
            'storagerack',
            'streetlight',
            'supportarch',
            'supportbeam',
            'supportcolumn',
            'table',
            'tireassembly',
            'toolbox',
            'trashcan',
            'tree',
            'tunnel',
            'tv',
            'vendingmachine',
            'vent',
            'ventilationgrill',
            'ventpipe',
            'vine',
            'walkway',
            'wall',
            'wallarch',
            'wallarchway',
            'walllight',
            'walls',
            'window',
            'windowframe',
            'woodenbox',
            'woodenpallet',
            'woodenplank',
            'free',
        ],
        empty_label=162,
        mask_camera_key='mask_camera',
        mask_lidar_key='mask_lidar',
        occ_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/',
        path_template='{scene_name}/{token}/labels.npz',
        semantics_key='semantics',
        type='LoadOcc3DFromFile'),
    dict(
        ida_aug_conf=dict(
            H=640,
            W=640,
            bot_pct_lim=(
                0.0,
                0.0,
            ),
            final_dim=(
                640,
                640,
            ),
            rand_flip=True,
            resize_lim=(
                1.0,
                1.0,
            ),
            rot_lim=(
                0.0,
                0.0,
            )),
        training=True,
        type='RandomTransformImage'),
    dict(
        bda_aug_conf=dict(flip_dx_ratio=0.5, flip_dy_ratio=0.5),
        empty_label=162,
        type='RandomTransformOcc'),
    dict(
        meta_keys=(
            'sample_idx',
            'sample_token',
            'scene_name',
            'filename',
            'ori_shape',
            'img_shape',
            'pad_shape',
            'ego2occ',
            'ego2img',
            'ego2lidar',
            'img_timestamp',
        ),
        type='PackOcc3DInputs'),
]
val_cfg = dict(type='ValLoop')
val_dataloader = dict(
    batch_size=1,
    collate_fn=dict(type='pseudo_collate'),
    dataset=dict(
        ann_file='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/val.pkl',
        classes=[
            'car',
            'truck',
            'construction_vehicle',
            'bus',
            'trailer',
            'barrier',
            'motorcycle',
            'bicycle',
            'pedestrian',
            'traffic_cone',
        ],
        data_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/',
        dataset_cfg=dict(
            cam_types=[
                'CAM_LEFT',
                'CAM_BACK',
                'CAM_FRONT',
                'CAM_BOTTOM',
                'CAM_TOP',
                'CAM_RIGHT',
            ],
            class_names=[
                'others',
                'acousticceiling',
                'airconditiongvent',
                'atm',
                'awning',
                'bakerycounter',
                'bannersign',
                'barcounter',
                'barrel',
                'barshelf',
                'beam',
                'bed',
                'bench',
                'bleach',
                'blinds',
                'boatcovered',
                'book',
                'boxedfood',
                'boxedlaundrydetergent',
                'bp',
                'building',
                'buildingtunnel',
                'cabinet',
                'cabinets',
                'cable',
                'cabledrum',
                'cameraactor',
                'car',
                'carpet',
                'cashregister',
                'ceiling',
                'cementcolumn',
                'chain',
                'chair',
                'chairs',
                'chasis',
                'childactor',
                'chipscylinder',
                'cieling',
                'cleaningpowder',
                'column',
                'concretebarricade',
                'couch',
                'counch',
                'cranerail',
                'cucumbers',
                'cupboard',
                'curtain',
                'decorativecookiebox',
                'desk',
                'donuts',
                'door',
                'doorway',
                'dumpster',
                'elevator',
                'elevatordoor',
                'fabricsoftener',
                'fence',
                'fencecolumn',
                'floor',
                'flowerpot',
                'fluorescentlight',
                'foldingscreen',
                'frozenfoodbagged',
                'fryingoil',
                'garagedoor',
                'garbagecan',
                'gate',
                'grass',
                'ground',
                'handtruck',
                'heater',
                'instancedfoliageactor',
                'ivy',
                'laboratorytable',
                'lamp',
                'largeconduit',
                'light',
                'lightbulbstring',
                'meatchub',
                'metalcieling',
                'metalfloor',
                'metalhandrail',
                'metalpanel',
                'metalpillar',
                'metalplatform',
                'metalpole',
                'metalramp',
                'metalstair',
                'metalsupport',
                'metaltop',
                'mirror',
                'monitor',
                'oranges',
                'overheadcrane',
                'painting',
                'papertowels',
                'petfoodtub',
                'piano',
                'picture',
                'pictureframe',
                'pillar',
                'pipe',
                'plant',
                'plantholder',
                'platform',
                'post',
                'railgatebase',
                'railing',
                'railtrack',
                'railtrackend',
                'receptiondesk',
                'refrigerator',
                'road',
                'robotarm',
                'rock',
                'rollingcabinet',
                'roof',
                'rug',
                'saucebottle',
                'screendivider',
                'seat',
                'sewergrate',
                'shampoo',
                'shelf',
                'shoppingbags',
                'sidewalk',
                'sign',
                'sky',
                'sodabottle',
                'sofa',
                'stair',
                'stairs',
                'staticmeshactor',
                'storagerack',
                'streetlight',
                'supportarch',
                'supportbeam',
                'supportcolumn',
                'table',
                'tireassembly',
                'toolbox',
                'trashcan',
                'tree',
                'tunnel',
                'tv',
                'vendingmachine',
                'vent',
                'ventilationgrill',
                'ventpipe',
                'vine',
                'walkway',
                'wall',
                'wallarch',
                'wallarchway',
                'walllight',
                'walls',
                'window',
                'windowframe',
                'woodenbox',
                'woodenpallet',
                'woodenplank',
                'free',
            ],
            empty_label=162,
            num_views=6,
            occ_io=dict(
                mask_camera_key='mask_camera',
                mask_lidar_key='mask_lidar',
                path_template='{scene_name}/{token}/labels.npz',
                semantics_key='semantics'),
            pc_range=[
                -20.0,
                -20.0,
                -3.0,
                20.0,
                20.0,
                5.0,
            ],
            ray=dict(
                lidar=dict(
                    azimuth_step_deg=1.0,
                    mode='nuscenes_default',
                    pitch_angles=None),
                max_origins=8,
                num_workers=8,
                origin_xy_bound=39.0),
            voxel_size=[
                0.05,
                0.05,
                0.05,
            ]),
        modality=dict(
            use_camera=True,
            use_external=True,
            use_lidar=False,
            use_map=False,
            use_radar=False),
        occ_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/',
        pipeline=[
            dict(
                color_type='color',
                imdecode_backend='pillow',
                to_float32=False,
                type='LoadMultiViewImageFromFiles'),
            dict(
                cam_types=[
                    'CAM_LEFT',
                    'CAM_BACK',
                    'CAM_FRONT',
                    'CAM_BOTTOM',
                    'CAM_TOP',
                    'CAM_RIGHT',
                ],
                imdecode_backend='pillow',
                sweeps_num=8,
                test_mode=True,
                type='LoadMultiViewImageFromMultiSweeps'),
            dict(
                ida_aug_conf=dict(
                    H=640,
                    W=640,
                    bot_pct_lim=(
                        0.0,
                        0.0,
                    ),
                    final_dim=(
                        640,
                        640,
                    ),
                    rand_flip=True,
                    resize_lim=(
                        1.0,
                        1.0,
                    ),
                    rot_lim=(
                        0.0,
                        0.0,
                    )),
                training=False,
                type='RandomTransformImage'),
            dict(
                meta_keys=(
                    'sample_idx',
                    'sample_token',
                    'scene_name',
                    'filename',
                    'ori_shape',
                    'img_shape',
                    'pad_shape',
                    'ego2occ',
                    'ego2img',
                    'ego2lidar',
                    'img_timestamp',
                ),
                type='PackOcc3DInputs'),
        ],
        test_mode=True,
        type='TartangroundOcc3DDataset'),
    num_workers=0,
    persistent_workers=False,
    sampler=dict(shuffle=False, type='DefaultSampler'))
val_evaluator = dict(
    ann_file='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/val.pkl',
    class_names=[
        'others',
        'acousticceiling',
        'airconditiongvent',
        'atm',
        'awning',
        'bakerycounter',
        'bannersign',
        'barcounter',
        'barrel',
        'barshelf',
        'beam',
        'bed',
        'bench',
        'bleach',
        'blinds',
        'boatcovered',
        'book',
        'boxedfood',
        'boxedlaundrydetergent',
        'bp',
        'building',
        'buildingtunnel',
        'cabinet',
        'cabinets',
        'cable',
        'cabledrum',
        'cameraactor',
        'car',
        'carpet',
        'cashregister',
        'ceiling',
        'cementcolumn',
        'chain',
        'chair',
        'chairs',
        'chasis',
        'childactor',
        'chipscylinder',
        'cieling',
        'cleaningpowder',
        'column',
        'concretebarricade',
        'couch',
        'counch',
        'cranerail',
        'cucumbers',
        'cupboard',
        'curtain',
        'decorativecookiebox',
        'desk',
        'donuts',
        'door',
        'doorway',
        'dumpster',
        'elevator',
        'elevatordoor',
        'fabricsoftener',
        'fence',
        'fencecolumn',
        'floor',
        'flowerpot',
        'fluorescentlight',
        'foldingscreen',
        'frozenfoodbagged',
        'fryingoil',
        'garagedoor',
        'garbagecan',
        'gate',
        'grass',
        'ground',
        'handtruck',
        'heater',
        'instancedfoliageactor',
        'ivy',
        'laboratorytable',
        'lamp',
        'largeconduit',
        'light',
        'lightbulbstring',
        'meatchub',
        'metalcieling',
        'metalfloor',
        'metalhandrail',
        'metalpanel',
        'metalpillar',
        'metalplatform',
        'metalpole',
        'metalramp',
        'metalstair',
        'metalsupport',
        'metaltop',
        'mirror',
        'monitor',
        'oranges',
        'overheadcrane',
        'painting',
        'papertowels',
        'petfoodtub',
        'piano',
        'picture',
        'pictureframe',
        'pillar',
        'pipe',
        'plant',
        'plantholder',
        'platform',
        'post',
        'railgatebase',
        'railing',
        'railtrack',
        'railtrackend',
        'receptiondesk',
        'refrigerator',
        'road',
        'robotarm',
        'rock',
        'rollingcabinet',
        'roof',
        'rug',
        'saucebottle',
        'screendivider',
        'seat',
        'sewergrate',
        'shampoo',
        'shelf',
        'shoppingbags',
        'sidewalk',
        'sign',
        'sky',
        'sodabottle',
        'sofa',
        'stair',
        'stairs',
        'staticmeshactor',
        'storagerack',
        'streetlight',
        'supportarch',
        'supportbeam',
        'supportcolumn',
        'table',
        'tireassembly',
        'toolbox',
        'trashcan',
        'tree',
        'tunnel',
        'tv',
        'vendingmachine',
        'vent',
        'ventilationgrill',
        'ventpipe',
        'vine',
        'walkway',
        'wall',
        'wallarch',
        'wallarchway',
        'walllight',
        'walls',
        'window',
        'windowframe',
        'woodenbox',
        'woodenpallet',
        'woodenplank',
        'free',
    ],
    empty_label=162,
    focus_eval=dict(
        ema_momentum=0.9,
        enabled=True,
        fallback='rare_classes',
        freq_thr=0.02,
        max_classes=24,
        min_classes=8,
        policy='ema_freq',
        rare_classes=[
            13,
            26,
            50,
            65,
            76,
            140,
            134,
            112,
            56,
            39,
            32,
            5,
            29,
            55,
            19,
            96,
            37,
            66,
            106,
            129,
            53,
            61,
            23,
            92,
        ]),
    mask_camera_key='mask_camera',
    mask_lidar_key='mask_lidar',
    miou_num_workers=32,
    occ_path_template='{scene_name}/{token}/labels.npz',
    occ_root='/root/wjl/OPUS_mmcv2/data/TartanGround_Indoor/gts/',
    pc_range=[
        -20.0,
        -20.0,
        -3.0,
        20.0,
        20.0,
        5.0,
    ],
    semantics_key='semantics',
    type='Occ3DMetric',
    use_camera_mask=True,
    voxel_size=[
        0.05,
        0.05,
        0.05,
    ])
visualizer = dict(
    type='Visualizer', vis_backends=[
        dict(type='TensorboardVisBackend'),
    ])
voxel_size = [
    0.05,
    0.05,
    0.05,
]
work_dir = '/root/wjl/OPUS_mmcv2/.tmp_grad_debug'
